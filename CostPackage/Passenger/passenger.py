...
dict_seats = df_seats[['AircraftType', 'HighScenario', 'BaseScenario', 'LowScenario']].set_index('AircraftType').to_dict(orient='index')
...
def get_passengers(aircraft_type: str, scenario: str = None, load_factor: float = None) -> int:
    ...
    seats = dict_seats[aircraft_cluster][entry_scenario]
    if load_factor is not None:
        ...
        if is_wide_body(aircraft_cluster):
            return round(seats * .85)
        else:
            if entry_scenario == "LowScenario":
